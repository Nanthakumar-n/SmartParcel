...
import type { HubRow, HubWithMetrics } from '@/lib/db/hubs';

interface HubTableProps {
  initialHubs: HubWithMetrics[];
}
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
        <Table>
          <TableHeader className="bg-slate-50 border-b border-slate-200">
            <TableRow>
              <TableHead className="w-[110px] font-bold text-slate-700">Hub Code</TableHead>
              ...
              <TableHead className="font-bold text-slate-700">In-Station Trucks</TableHead>
              <TableHead className="font-bold text-slate-700">Incoming</TableHead>
              <TableHead className="font-bold text-slate-700">Outgoing</TableHead>
              <TableHead className="font-bold text-slate-700">Contact</TableHead>
              <TableHead className="w-[90px] font-bold text-slate-700">Status</TableHead>
              <TableHead className="w-[70px] text-right font-bold text-slate-700">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
                ...
                <TableRow key={hub.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1.5 font-bold text-slate-900 text-xs">
                      ...
                      <span>{hub.in_station_trucks_count ?? 0}</span>
                      <span className="text-[11px] text-slate-500 font-normal">
                        {hub.in_station_trucks_count === 1 ? 'truck' : 'trucks'}
                      </span>
                    </div>
                  </TableCell>
                  ...
                  {/* Incoming Trucks / LRs */}
                  <TableCell>
                    {(hub.incoming_trucks_count ?? 0) > 0 || (hub.incoming_lrs_count ?? 0) > 0 ? (
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-1 text-xs font-semibold text-emerald-700">
                          <ArrowDownLeft className="h-3.5 w-3.5 text-emerald-600 shrink-0" />
                          <span>{hub.incoming_trucks_count} {hub.incoming_trucks_count === 1 ? 'truck' : 'trucks'}</span>
                        </div>
                        <span className="text-[10px] text-slate-500 pl-4 font-medium">
                          {hub.incoming_lrs_count} {hub.incoming_lrs_count === 1 ? 'LR' : 'LRs'} arriving
                        </span>
                      </div>
                    ) : (
                  ...
                  {/* Outgoing Trucks / LRs */}
                  <TableCell>
                    {(hub.outgoing_trucks_count ?? 0) > 0 || (hub.outgoing_lrs_count ?? 0) > 0 ? (
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-1 text-xs font-semibold text-blue-700">
                          ...
                          <span>{hub.outgoing_trucks_count} {hub.outgoing_trucks_count === 1 ? 'truck' : 'trucks'}</span>
                        </div>
                        <span className="text-[10px] text-slate-500 pl-4 font-medium">
                          {hub.outgoing_lrs_count} {hub.outgoing_lrs_count === 1 ? 'LR' : 'LRs'} loaded
                        </span>
                      </div>
                    ) : (
                      <span className="text-xs text-slate-400 font-mono">—</span>
